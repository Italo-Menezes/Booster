...
class PointsController {
    async show(request, response) {
        ...
        try {
            const Point = await index_1.point.findByPk(id);
            if (!Point) {
                return response.status(400).json({ message: "Point not found" });
            }
            /* trazer tudo referente ao id e que venha so o title  */
            const items = await index_1.points_items.findAll({
                where: { point_id: id },
                include: [
                    {
                        model: index_1.item,
    ...
    async create(request, response) {
        ...
        const trx = await sequelize_1.sequelize.transaction();
        const Point = await index_1.point.create({
        ...
        await index_1.points_items.bulkCreate(pointItems, { transaction: trx });
        const variable = await index_1.point.findAll();
        await trx.commit();
    ...
    async index(request, response) {
        try {
            ...
            const Points = await index_1.point.findAll({
                include: [
                    {
                        model: index_1.points_items,
                        as: "points_items",
